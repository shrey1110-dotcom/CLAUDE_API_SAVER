...
- **Claude supplied-context benchmark — iteration 2, Claude profile** (2026-06-11, 5 tasks, Graphify v0.8.36 Gemini, 3 repeats/arm, no MCP): with the new `--profile claude` pack (budget 900), repo-context supplied context was **70%+ smaller** than Graphify-derived context on all 5 tasks (79–88% reduction) with **equal or better quality on 5/5 tasks** (2 outright wins, 3 ties at 10.0). The Graphify arm is "Graphify-derived context" (graph.json keyword extraction + GRAPH_REPORT.md; `graphify query` unavailable/hanging in 0.8.36). Exact Claude token usage not captured (Cursor IDE, no API counter) — Claude token-savings claim NOT ALLOWED; no "beats Graphify overall" claim. Iteration 1 (ultra profile: 94–97% smaller but quality only 2/5) is preserved at `.mcp-benchmarks/claude/iterations/iteration-1/`. See `.mcp-benchmarks/claude/summary.md`.
