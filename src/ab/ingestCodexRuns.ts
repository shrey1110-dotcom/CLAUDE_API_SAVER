...
import { mergeUsageWithTotal, parseCodexUsageFromOutput } from "./adapters/codexUsage.js";
import {
  AB_CODEX_RUNS_DIR,
  ensureAbDirectories,
  readCurrentPlan,
...
function parseModeFromDirName(name: string): { mode: AbMode; index: number } | null {
  const match = name.match(/-(no_mcp|context_broker_locked|context_broker)-(\d+)$/);
  if (!match) return null;
  return { mode: match[1] as AbMode, index: Number(match[2]) };
}

function listRunFolders(): RunFolder[] {
  const root = resolveAbPath(AB_CODEX_RUNS_DIR);
  ...
  for (const name of fs.readdirSync(root)) {
    ...
    if (readRunUsage(folder)?.clientTotalTokens === undefined) continue;
    folders.push(folder);
  }
...
function latestBatchFolders(folders: RunFolder[]): RunFolder[] {
  ...
  const batchKey = (dir: string): string => {
    ...
    const match = base.match(/^(.*)-(no_mcp|context_broker_locked|context_broker)-\d+$/);
    return match?.[1] ?? base;
  };
...
function readRunUsage(folder: RunFolder): ReturnType<typeof mergeUsageWithTotal> | null {
  let runJson: { parsedUsage?: ReturnType<typeof mergeUsageWithTotal> } | null = null;
  if (fs.existsSync(folder.runJsonPath)) {
    runJson = JSON.parse(fs.readFileSync(folder.runJsonPath, "utf8")) as {
      parsedUsage?: ReturnType<typeof mergeUsageWithTotal>;
    };
  }
  if (runJson?.parsedUsage) return mergeUsageWithTotal(runJson.parsedUsage);
  ...
  return parsed ? mergeUsageWithTotal(parsed) : null;
}
...
function readRunMcpTelemetry(folder: RunFolder): {
  ...
  const stdout = readText(path.join(folder.dir, "stdout.txt"));
  const tools = extractMcpToolsFromCodexTranscript(`${transcript}\n${stdout}`);
...
function buildResultFromFolders(plan: AbTestPlan, mode: AbMode, folders: RunFolder[]): AbRunResult | null {
  ...
  const usages = folders.map((folder) => readRunUsage(folder)).filter((value) => value !== null);
  ...
  const isMcpMode = mode === "context_broker" || mode === "context_broker_locked";
  const perRunMcpTokens = folders.map((folder) => {
    ...
    return mcp.mcpEstimatedOutputTokens > 0 ? mcp.mcpEstimatedOutputTokens : isMcpMode ? 594 : 0;
  });
  ...
  if (isMcpMode && mcp) {
    if (mcp.mcpToolsUsed.length === 0) {
      for (const folder of folders) {
        const tools = extractMcpToolsFromCodexTranscript(
          `${readText(path.join(folder.dir, "transcript.md"))}\n${readText(path.join(folder.dir, "stdout.txt"))}`,
        );
        for (const tool of tools) {
  ...
  const latestUsage = usages[usages.length - 1] ?? null;

  return {
    id: `${plan.id}-${mode}-ingested-${Date.now()}`,
    planId: plan.id,
    mode,
    client: plan.client,
    ...
    usageParsed: usages.length > 0,
    usageManuallyEntered: false,
    notes: quality?.note ?? `Ingested ${folders.length} Codex run folder(s) for ${mode}.`,
  };
}

export function ingestCodexRunsForPlan(plan: AbTestPlan): AbRunResult[] {
...
function main(): void {
  ...
  ensureAbDirectories();
  const ingested = ingestCodexRunsForPlan(plan);
  console.log(`Ingested ${ingested.length} mode result(s) from ${resolveAbPath(AB_CODEX_RUNS_DIR)}.`);
  for (const result of ingested) {
    console.log(
      `- ${result.mode}: repeats=${result.clientTotalTokenRepeats?.length ?? 0}, quality=${result.answerQuality ?? "-"}, found=${result.foundExpectedFiles ?? "-"}`,
    );
  }
}

const isMain = process.argv[1]?.endsWith("ingestCodexRuns.js");
if (isMain) {
  main();
}
